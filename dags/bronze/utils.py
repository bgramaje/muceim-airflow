...
def merge_from_csv(table_name, url):
    ...
    try:
        ...
        if is_transaction_error:
            print(f"  ⚠️ Transaction error detected - forcing new connection for next task")
            try:
...
def merge_from_json(table_name, url, key_columns=None):
    ...
    if key_columns is None:
        ...
        if missing:
            raise ValueError(f"Key columns {missing} not found in table metadata.")
...
def _filter_json_urls(table_name: str, urls: list[str]):
    ...
    try:
        ...
        if not table_exists:
            print(f"[TASK] Table {table_name} does not exist. Returning all URLs.")
            return urls
    ...
    try:
        ...
        ingested_urls = set(ingested_df['source_url'].tolist()) if not ingested_df.empty else set()
        print(f"[TASK] Found {len(ingested_urls)} already ingested URLs (out of {len(urls)} checked)")
    except Exception as e:
    ...
    print(f"[TASK] Filtered result: {len(new_urls)} new URLs to ingest (skipping {len(urls) - len(new_urls)} already ingested)")
    
    if len(new_urls) == 0:
        print(f"[TASK] ⚠️  All URLs have already been ingested. No new data to process.")
    else:
        print(f"[TASK] ✅ URLs to ingest: {new_urls[:3]}{'...' if len(new_urls) > 3 else ''}")
...
def _filter_csv_urls(table_name: str, urls: list[str]):
    ...
    try:
        ...
        ingested_urls = set(ingested_df['source_file'].tolist()) if not ingested_df.empty else set()
        print(f"[TASK] Found {len(ingested_urls)} already ingested URLs (out of {len(urls)} checked)")
    except Exception as e:
        print(f"[TASK] Warning: Could not check existing URLs: {e}")
        print(f"[TASK] Proceeding as if no URLs are ingested")
