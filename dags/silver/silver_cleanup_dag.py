"""
DAG for cleaning up Silver layer data.
Allows cleaning files from RustFS bucket and optionally dropping Silver tables.

Usage:
    Trigger with parameters:
    - source: Which source to clean ('mitma', 'ine')
    - cleanup_rustfs: If True, deletes files from RustFS bucket
    - cleanup_tables: If True, drops Silver tables (use with caution!)
...
with DAG(
    ...
    params={
        "source": Param(
            ...
            default="all",
            enum=["all", "mitma", "ine"],
            description="Which data source to clean (all, mitma, ine)"
        ),
        ...
        "cleanup_tables": Param(
            type="boolean",
            default=False,
            description="⚠️ Drop Silver tables (DANGER: deletes tables completely!)"
        ),
        "dataset": Param(
            type="string",
            default="all",
    ...
    description="⚠️ Cleanup DAG for Silver layer - Manual trigger only",
    default_args={
        "retries": 1,
    ...
    def tables_cleanup_group():
        ...
        list_tables = list_silver_tables.override(task_id="list_tables")()
        
        drop_tables = drop_silver_tables.override(task_id="drop_tables")(
