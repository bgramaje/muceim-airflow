...
for gravity model analysis. These run locally as they require 
...
def GOLD_generate_mismatch_distribution(
    ...
    """
    print("[TASK] Generating mismatch distribution")
    con = get_ducklake_connection()
    df_mm_ratios = con.execute(f"""
    ...
    """).fetchdf()
    mm_ratios = np.asarray(df_mm_ratios["mm_ratio"].to_list())
...
def GOLD_generate_table(
    ...
    """
    print("[TASK] Generating table of gravity model")
    con = get_ducklake_connection()
    df = con.execute(f"""
        WITH mismatch_filtered AS (
            SELECT
...
def GOLD_generate_mismatch_map(
    ...
    print("[TASK] Generating mismatch map")
    con = get_ducklake_connection()
