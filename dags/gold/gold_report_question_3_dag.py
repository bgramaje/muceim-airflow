...
def generate_directory(**context):
    ...
    s3_key = f"gold/question3/{directory_name}/info.txt"
    s3.load_string(
        string_data=content,
        key=s3_key,
...
with DAG(
    ...
    description="Gold layer Question 3 report generation - Functional Type classification. Manual execution only with parameters",
    params=get_params(),
    default_args={
        "retries": 3,
    ...
    start >> infra_group >> [func_type_dist, func_type_map] >> done
