...
Explicar cada fuente de datos y que extraemos de ella. Como los extraemos (APIs...)
...
\begin{itemize}
    ...
    \item \texttt{bronze\_mitma\_people\_day\_municipios}: Daily population presence by zone, age group, sex, and travel frequency category.
    \item \texttt{bronze\_mitma\_overnight\_stay\_municipios}: Overnight stay flows linking residence zones to stay zones.
...
\textbf{\texttt{silver\_people\_day} and \texttt{silver\_overnight\_stay}:} These tables maintain the same granularity as their Bronze counterparts but with proper type casting and date normalization. They are partitioned by \texttt{fecha} for efficient temporal queries.
...
\begin{figure}[H]
    ...
    \node[task] (people_urls) at (0, 4.5) {people\_urls};
    \node[task] (people_filter) at (0, 3.5) {people\_filter};
    \node[task] (people_check) at (0, 2.5) {check\_urls};
    \node[task] (people_create) at (0, 1.5) {people\_create};
    \node[task] (people_insert) at (0, 0.5) {people\_insert};
    
    \node[task] (zonif_urls) at (3, 4.5) {zonif\_urls};
    \node[task] (zonif_check) at (3, 3.5) {check\_urls};
    \node[task] (zonif) at (3, 2.5) {zonif};
    
    \node[task] (ine_relations) at (6, 4.5) {relations};
    
    \node[task] (done) at (1.5, -3.5) {done};
    
    % Arrows
    \draw[arrow] (infra) -- (od_urls);
    \draw[arrow] (infra) -- (people_urls);
    ...
    \draw[arrow] (people_urls) -- (people_filter);
    \draw[arrow] (people_filter) -- (people_check);
    \draw[arrow] (people_check) -- (people_create);
    \draw[arrow] (people_create) -- (people_insert);
    
    \draw[arrow] (zonif_urls) -- (zonif_check);
    \draw[arrow] (zonif_check) -- (zonif);
    
    \draw[arrow] (od_cleanup) -- (done);
    \draw[arrow] (people_insert) -- (done);
    \draw[arrow] (zonif) -- (done);
    \draw[arrow] (ine_relations) -- (done);
    
    \end{tikzpicture}
    \caption{Bronze MITMA DAG workflow showing parallel task groups for OD, People Day, Zonification, and INE Relations processing.}
...
\begin{figure}[H]
    ...
    \node[task] (people_create) at (1.5, 3.5) {people\_create};
    \node[task] (people_insert) at (1.5, 2.5) {people\_insert};
    ...
    \draw[arrow] (start) -- (people_create);
...
\begin{itemize}
    ...
    \item \textbf{Primary Key:} No formal primary key (all columns as VARCHAR)
...
\begin{longtable}{p{3cm} p{2cm} p{1.5cm} p{1.8cm} p{6cm}}
    ...
    \midrule
    fecha & VARCHAR & -- & No & Date in YYYYMMDD format\\
...
\subsubsection*{Data Sheet: \texttt{bronze\_mitma\_people\_day\_municipios}}
\begin{itemize}
    \item \textbf{Description:} Raw daily population presence data by zone, age group, sex, and travel frequency.
    \item \textbf{Grain:} One row per stay zone, age group, sex, travel frequency category, and date.
    \item \textbf{Primary Key:} No formal primary key
    \item \textbf{Sources:} MITMA Estudios Básicos RSS feeds
    \item \textbf{Tracking:} \texttt{source\_file} column contains URL of source CSV file
\end{itemize}

\begin{longtable}{p{3cm} p{2cm} p{1.5cm} p{1.8cm} p{6cm}}
    \caption{Technical Data Sheet for the bronze\_mitma\_people\_day\_municipios Table} \\
    \toprule
    \textbf{Column} & \textbf{Type} & \textbf{Unit} & \textbf{Nullable} & \textbf{Description} \\
    \midrule
    fecha & VARCHAR & -- & No & Date in YYYYMMDD format\\
    zona\_pernoctacion & VARCHAR & -- & No & Zone where individuals stay overnight\\
    edad & VARCHAR & -- & Yes & Age group category\\
    sexo & VARCHAR & -- & Yes & Sex category\\
    numero\_viajes & VARCHAR & -- & Yes & Daily travel frequency category\\
    personas & VARCHAR & persons & No & Number of individuals (as string)\\
    source\_file & VARCHAR & -- & No & URL of source CSV file\\
    loaded\_at & TIMESTAMP & -- & No & Timestamp of ingestion\\
    \bottomrule
\end{longtable}

\subsubsection*{Data Sheet: \texttt{bronze\_mitma\_overnight\_stay\_municipios}}
\begin{itemize}
    \item \textbf{Description:} Raw overnight stay flows between residence and stay zones.
    \item \textbf{Grain:} One row per stay zone, residence zone, and date.
    \item \textbf{Primary Key:} No formal primary key
    \item \textbf{Sources:} MITMA Estudios Básicos RSS feeds
    \item \textbf{Tracking:} \texttt{source\_file} column contains URL of source CSV file
\end{itemize}
...
\begin{longtable}{p{3cm} p{2cm} p{1.5cm} p{1.8cm} p{6cm}}
    \caption{Technical Data Sheet for the bronze\_mitma\_overnight\_stay\_municipios Table} \\
    \toprule
    \textbf{Column} & \textbf{Type} & \textbf{Unit} & \textbf{Nullable} & \textbf{Description} \\
    \midrule
    fecha & VARCHAR & -- & No & Date in YYYYMMDD format\\
    zona\_pernoctacion & VARCHAR & -- & No & Zone where overnight stay occurs\\
    zona\_residencia & VARCHAR & -- & No & Residence zone of individuals\\
    personas & VARCHAR & persons & No & Number of individuals (as string)\\
    source\_file & VARCHAR & -- & No & URL of source CSV file\\
    loaded\_at & TIMESTAMP & -- & No & Timestamp of ingestion\\
    \bottomrule
...
\begin{itemize}
    ...
    \item \textbf{Primary Key:} ID (implicit)
...
\begin{itemize}
    ...
    \item \textbf{Primary Key:} Codigo (INE code)
...
\begin{itemize}
    ...
    \item \textbf{Primary Key:} municipio\_mitma, municipio\_ine (implicit)
...
\begin{itemize}
    ...
    \item \textbf{Primary Key:} date (implicit)
...
\begin{itemize}
    ...
    \item \textbf{Primary Key:} codigo\_ine, municipio\_mitma
...
\begin{itemize}
    ...
    \item \textbf{Primary Key:} id, year
...
\begin{itemize}
    ...
    \item \textbf{Primary Key:} origen\_zone\_id, destino\_zone\_id, fecha, residencia
...
\subsubsection*{Data Sheet: \texttt{silver\_overnight\_stay}}
\begin{itemize}
    \item \textbf{Description:} Overnight stay flows between residence and stay zones.
    \item \textbf{Grain:} One row per stay zone, residence zone, and date.
    \item \textbf{Primary Key:} zona\_pernoctacion, zona\_residencia, fecha
    \item \textbf{Sources:} MITMA Estudios Básicos
\end{itemize}

\begin{longtable}{p{3cm} p{2cm} p{1.5cm} p{1.8cm} p{6cm}}
    \caption{Technical Data Sheet for the silver\_overnight\_stay Table} \\
    \toprule
    \textbf{Column} & \textbf{Type} & \textbf{Unit} & \textbf{Nullable} & \textbf{Description}\\
    \midrule
    zona\_pernoctacion & VARCHAR & -- & No & Zone where overnight stay occurs\\
    ...
    personas & DOUBLE & persons & No & Number of individuals staying overnight\\
    \bottomrule
\end{longtable}

\subsubsection*{Data Sheet: \texttt{silver\_people\_day}}
\begin{itemize}
    \item \textbf{Description:} Daily population presence by demographic attributes and stay location.
    \item \textbf{Grain:} One row per stay zone, age group, sex, travel frequency category, and date.
    \item \textbf{Primary Key:} zona\_pernoctacion, edad, sexo, numero\_viajes, fecha
    \item \textbf{Sources:} MITMA
\end{itemize}

\begin{longtable}{p{3cm} p{2cm} p{1.5cm} p{1.8cm} p{6cm}}
    \caption{Technical Data Sheet for the silver\_people\_day Table} \\
    \toprule
    \textbf{Column} & \textbf{Type} & \textbf{Unit} & \textbf{Nullable} & \textbf{Description}\\
    \midrule
    zona\_pernoctacion & VARCHAR & -- & No & Zone where individuals stay overnight\\
    edad & VARCHAR & -- & No & Age group category\\
    sexo & VARCHAR & -- & No & Sex category\\
    numero\_viajes & VARCHAR & -- & No & Daily travel frequency category\\
    personas & DOUBLE & persons & No & Number of individuals present\\
    fecha & DATETIME & -- & No & Observation date\\
    \bottomrule
\end{longtable}
...
\begin{itemize}
    ...
    \item \textbf{Primary Key:} origin, destination
...
\begin{itemize}
    ...
    \item \textbf{Primary Key:} origen\_zone\_id, destino\_zone\_id, fecha
...
\begin{itemize}
    ...
    \item \textbf{Primary Key:} origin\_id, destination\_id, date, hour
...
\begin{itemize}
    ...
    \item \textbf{Grain:} One row per origin zone, destination zone, and date.
    \item \textbf{Primary Key:} origin\_id, destination\_id, date
...
\begin{itemize}
    ...
    \item \textbf{Grain:} One row per spatial zone.
    \item \textbf{Primary Key:} zone\_id
